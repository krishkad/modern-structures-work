

export const navLinks = [
    {
        label: "HOME",
        href: "/#home"
    },
    {
        label: "ABOUT US",
        href: "/#aboutus"
    },
    {
        label: "PROJECTS",
        href: "/#projects"
    },
    {
        label: "SERVICES",
        href: "/services"
    },
    {
        label: "CONTACT",
        href: "/#contact"
    },
];

export const carouselImg = [
    {
        img: "/affordable-housing/Bedroom_final.jpg",
    },
    {
        img: "/affordable-housing/Kyadondo_1BHK-fin01.jpg",
    },
    {
        img: "/affordable-housing/Kyadondo_2BHK-cam01_night_fin.jpg"
    },
    {
        img: "/affordable-housing/Kyadondo_Studio-cam01.jpg"
    },

];

export const projectLink = [
    {
        label: "ALL PROJECTS",
    },
    {
        label: "INTERIOR DESIGN",
    },
    {
        label: "ISOLATION",
    },
    {
        label: "TILING",
    },
    {
        label: "BUILDINGS",
    },
];


export const projectContent = [
    {
        path: 'affordable-housing',
        title: "AFFORDABLE HOUSING",
        content1: {
            img: "/affordable-housing/Bedroom_final.jpg",
            text: "Modern Structure has proposed a residential housing development project to be constructed in Kampala, Uganda with a total of 164 units."
        },
        content2: {
            img: "/affordable-housing/Kyadondo_1BHK-fin01.jpg",
            text: "This proposed project will be built on a plot size of 2 acres with a total constructed area of 8500 square metres. Modern Structure will be responsible for the design and consultation parts of this project."
        },
        imgs: [
            {
                path: "/affordable-housing/Bedroom_final.jpg",
            },
            {
                path: "/affordable-housing/Kyadondo_1BHK-fin01.jpg",
            },
            {
                path: "/affordable-housing/Kyadondo_2BHK-cam01_night_fin.jpg"
            },
            {
                path: "/affordable-housing/Kyadondo_Studio-cam01.jpg"
            },
            // {
            //     path: "/affordable-housing/5.png"
            // },
            // {
            //     path: "/affordable-housing/6.png"
            // },
            // {
            //     path: "/affordable-housing/bhk1.jpg"
            // },

        ]
    },
    {
        path: 'impala-dreams',
        title: "IMPALA DREAMS",
        content1: {
            img: "/IMPALA-DREAMS/final-1-ps.jpg",
            text: "Situated in Nairobi, Kenya Impala Dreams is a luxury residential maisonette project with dedicated amenities such as lawns, a patio area and a common gym and swimming pool. Each of the 7 units has a space of 280 square meters, with four bedrooms, with the Master Bedroom having a private jacuzzi, walk-in closet and dedicated balcony area."
        },
        content2: {
            img: "/IMPALA-DREAMS/1.jpg",
            text: "Impala Dreams was built on a plot size of 0.5 acres with a total constructed area of 1700 square metres. Modern Structure was responsible for the design and consultation parts of this project."
        },
        imgs: [
            {
                path: "/IMPALA-DREAMS/final-1-ps.jpg",
            },
            {
                path: "/IMPALA-DREAMS/1.jpg",
            },
            {
                path: "/IMPALA-DREAMS/8.jpg",
            },

        ]
    },
    {
        path: 'burhani-masjid',
        title: "BURHANI MASJID",
        content1: {
            img: "/BURHANI-MASJID/_MG_4284.jpg",
            text: "Burhani Masjid, located on Bukasa Road on Muyenga Hill in the capital city of Uganda - Kampala, is the mosque of the Bohra community. It is situated a few kilometres away from the city center and has a great view of the Lake Victoria. The Masjid stands tall and proud on the hill and it evokes grandeur and splendor. This structure houses the mosque complex, offices, community kitchen, madrasa and residential complexes."
        },
        content2: {
            img: "/BURHANI-MASJID/_MG_4305.jpg",
            text: "This property was constructed by Modern Structure on a plot of 1.5 acres with a total constructed area of 4500 square meters."
        },
        imgs: [
            {
                path: "/BURHANI-MASJID/_MG_4284.jpg",
            },
            {
                path: "/BURHANI-MASJID/_MG_4305.jpg",
            },
            {
                path: "/BURHANI-MASJID/masjid-edited.jpg",
            },
            {
                path: "/BURHANI-MASJID/ROD_6900.jpg",
            },
        ]
    },
    {
        path: 'modern-heights',
        title: "MODERN HEIGHTS",
        content1: {
            img: "/affordable-housing/KAMPALA-cam-3-final.jpg",
            text: "Modern Heights is a proposed residential project, situated on Bukasa road on Muyenga hill in the capital city of Uganda - Kampala. Situated near the city center and the Lake Victoria, Modern Heights is a premium lake-view condominium apartment project focusing on providing the best of luxury and comfort. A total 24 units, of 2-Bedroom or 3-Bedroom with kitchen and attached bathrooms, are spread across 6 floors. The property was also built with rooftop amenities, children's play areas and dedicated rooms for support staff."
        },
        content2: {
            img: "/affordable-housing/KAMPALA-cam-4-final.jpg",
            text: "Modern Heights was built on a plot size of 0.3 acres with a total constructed area of 4000 square meters. Modern Structure undertook both the design and the construction for this project."
        },
        imgs: [
            {
                path: "/MODERN-HEIGHTS/KAMPALA-cam-3-final.jpg",
            },
            {
                path: "/MODERN-HEIGHTS/KAMPALA-cam-4-final.jpg",
            },

        ]
    },
    {
        path: 'pearl-homes',
        title: "PEARL HOMES",
        content1: {
            img: "/PEARL-HOMES/PRL-56.jpg",
            text: "Pearl Homes is a residential project, situated at Rubaga, in the capital city of Uganda - Kampala. Pearl Homes is a premium condominium apartment project. A total of 11 units - of 2-Bedroom and 3-Bedroom apartments each with a kitchen, attached bathrooms and living rooms - are spread across 4 floors. The property has a lot of parking space for vehicles and a dedicated children's play area."
        },
        content2: {
            img: "/PEARL-HOMES/PRL-15.jpg",
            text: "Pearl Homes was built on a plot size of 0.25 acres with a total constructed area of 1300 square meters. Modern Structure was responsible the design and construction parts of this project."
        },
        imgs: [
            {
                path: "/PEARL-HOMES/DJI_0020.jpg",
            },
            {
                path: "/PEARL-HOMES/PRL-56.jpg",
            },
            // {
            //     path: "/PEARL-HOMES/DJI_0020-min.png",
            // },

        ]
    },
    {
        path: 'modern-developers',
        title: "MODERN DEVELOPERS",
        content1: {
            img: "/IMPALA-GARDENS/2.jpg",
            text: "Situated on Gataka Road, near Osupoko Estate in Nairobi, the capital city of Kenya; Impala Gardens is a luxury residential project in a great neighbourhood with quick access to amenities and institutions such as schools, hospitals and supermarkets. With a total of 98 units, Impala Gardens offers both 3-bedroom and 4-bedroom choices."
        },
        content2: {
            img: "/IMPALA-GARDENS/4.jpg",
            text: "Welcome to Modern Structures, where we transform dreams into reality through innovative real estate development. With a steadfast commitment to excellence and a keen eye for detail, we craft spaces that not only meet but exceed expectations. Our portfolio spans residential, commercial, and mixed-use developments, each project meticulously designed to offer unmatched quality and lasting value."
        },
        imgs: [
            {
                path: "/affordable-housing/Bedroom_final.jpg",
            },
            {
                path: "/affordable-housing/Kyadondo_1BHK-fin01.jpg",
            },
            {
                path: "/affordable-housing/Kyadondo_2BHK-cam01_night_fin.jpg"
            },
            {
                path: "/affordable-housing/Kyadondo_Studio-cam01.jpg"
            },
        ]
    },
    {
        path: 'impala-gardens',
        title: "IMPALA GARDENS",
        content1: {
            img: "/IMPALA-GARDENS/2.jpg",
            text: "Situated on Gataka Road, near Osupoko Estate in Nairobi, the capital city of Kenya; Impala Gardens is a luxury residential project in a great neighbourhood with quick access to amenities and institutions such as schools, hospitals and supermarkets. With a total of 98 units, Impala Gardens offers both 3-bedroom and 4-bedroom choices."
        },
        content2: {
            img: "/IMPALA-GARDENS/4.jpg",
            text: "Each unit is duplex, with the main bedrooms on the upper floor; The Master Bedroom has a dedicated balcony area, and the project has common amenities such as a children's play area, gym, swimming pool and grocery store.Impala Gardens was built on a plot size of 5 acres with a total constructed area of 16000 square metres. Modern Structure was responsible for the design and consultation parts of this project."
        },
        imgs: [
            {
                path: "/IMPALA-GARDENS/4BHK-Gataka20.jpg",
            },
            {
                path: "/IMPALA-GARDENS/front.jpg",
            },
            {
                path: "/IMPALA-GARDENS/gataka9.jpg",
            },
            {
                path: "/IMPALA-GARDENS/Impala-Dreams-Gataka-Drone-17th-sept1.jpg",
            },
            // {
            //     path: "/IMPALA-GARDENS/3.jpg",
            // },
            // {
            //     path: "/IMPALA-GARDENS/6.jpg",
            // },
            // {
            //     path: "/IMPALA-GARDENS/7.jpg",
            // },
            // {
            //     path: "/IMPALA-GARDENS/ImpalaDreamsGatakaDrone17thsept3.jpg",
            // },
        ]
    },
]



export const imgesArray = [
    {
        path: 'affordable-housing',
        images: [
            {
                path: "/affordable-housing/Bedroom_final.jpg",
            },
            {
                path: "/affordable-housing/Kyadondo_1BHK-fin01.jpg",
            },
            {
                path: "/affordable-housing/Kyadondo_2BHK-cam01_night_fin.jpg"
            },
            {
                path: "/affordable-housing/Kyadondo_Studio-cam01.jpg"
            },

        ]
    },
    {
        path: 'modern-developers',
        images: [
            {
                path: "/affordable-housing/Bedroom_final.jpg",
            },
            {
                path: "/affordable-housing/Kyadondo_1BHK-fin01.jpg",
            },
            {
                path: "/affordable-housing/Kyadondo_2BHK-cam01_night_fin.jpg"
            },
            {
                path: "/affordable-housing/Kyadondo_Studio-cam01.jpg"
            },
        ]
    },
    {
        path: 'impala-dreams',
        images: [
            {
                path: "/IMPALA-DREAMS/final-1-ps.jpg",
            },
            {
                path: "/IMPALA-DREAMS/1.jpg",
            },
            {
                path: "/IMPALA-DREAMS/8.jpg",
            },

        ]
    },
    {
        path: 'burhani-masjid',
        images: [
            {
                path: "/BURHANI-MASJID/_MG_4284.jpg",
            },
            {
                path: "/BURHANI-MASJID/_MG_4305.jpg",
            },
            {
                path: "/BURHANI-MASJID/masjid-edited.jpg",
            },
            {
                path: "/BURHANI-MASJID/ROD_6900.jpg",
            },

        ]
    },
    {
        path: 'modern-heights',
        images: [
            {
                path: "/MODERN-HEIGHTS/KAMPALA-cam-3-final.jpg",
            },
            {
                path: "/MODERN-HEIGHTS/KAMPALA-cam-4-final.jpg",
            },

        ]
    },
    {
        path: 'pearl-homes',
        images: [
            {
                path: "/PEARL-HOMES/DJI_0020.jpg",
            },
            {
                path: "/PEARL-HOMES/PRL-56.jpg",
            },

        ]
    },
    {
        path: 'impala-gardens',
        images: [
            {
                path: "/IMPALA-GARDENS/4BHK-Gataka20.jpg",
            },
            {
                path: "/IMPALA-GARDENS/front.jpg",
            },
            {
                path: "/IMPALA-GARDENS/gataka9.jpg",
            },
            {
                path: "/IMPALA-GARDENS/Impala-Dreams-Gataka-Drone-17th-sept1.jpg",
            },

        ]
    },
]